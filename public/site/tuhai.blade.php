<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="Манай улсын ууган их сургууль">
    <link rel="stylesheet" href="tuhai.css">
   
    <title>Хөдөө аж ахуйн их сургууль</title>
</head>
<body>
    <header>
        <nav id="navbar">
            <div class="container">
            <h1 class="logo"><a href="index.html"><img src="img/logo.png" alt="зургийн тайлбар" width="300px" ></a>
            </h1>
            <ul>
                <li><a class="current" href="index.html">Сургууль</a></li>
                <li><a href="#">Хөтөлбөр</a></li>
                <li><a href="#">Багш, ажиллагсад</a></li>
                <li><a href="#">Төгсөгчид</a></li>
            </ul>
            </div>

        </nav>
    </header>
    <header>
        <nav id="menu">
            <div id="container">
            <ul>
                    <li><a class="current" href="index.html">Нүүр </a></li>
                    <li><a href="tuhai.html">Бидний тухай</a></li>
                    <li><a href="elselt.html">Элсэлт</a></li>
                    <li><a href="#">Сургалт</a></li>
                    <li><a href="#">Судалгаа, инноваци</a></li>
                    <li><a href="#">Чанарын удирдлага</a></li>
                    <li><a href="#">Хамтын ажиллагаа</a></li>
                    <li><a href="#">Сургуулийн орчин</a></li>
                    <li><a href="#">Холбоо барих</a></li> 
            </ul>
                <input type="text"  placeholder="Хайх..">
                </div>
    
        </nav>     
    </header>  
    
    <!-- Мэдээлэл хэсэг 1 -->
    <section id="container1">
        <div class="item item-1"><h3 style="text-align: center; color: green;">Алтан гадас одонт ХААИС-ийн танилцуулга үүсэл хөгжил, түүхэн замнал</h3> <br>
            <hr>
            <br>
        <p class="text-indent">
            ХААИС нь одоогоос 65 жилийн тэртээ анх мал эмнэлгийн анги нээгдэж анхны хичээлээ эхэлж байснаар түүхэн замналынхаа анхны хуудсыг нээж байсан бол, өнөөдөр бүрэлдэхүүний 6 сургууль, харъяа тэргүүлэх 3 эрдэм шинжилгээний хүрээлэн, сургалт дадлагын баазтайгаар үйл ажиллагаа явуулж, тэдгээрт 1000 гаруй эрдэмтэн багш, эрдэм шинжилгээний ажилтан, ажиллагсад ажиллаж, бакалавр, магистр, докторын шаталсан сургалтанд 10.000 гаруй оюутан суралцаж байгаа эрдэм шинжилгээ-сургалт-үйлдвэрлэлийн гурвалсан холбоо бүхий үндэсний хэмжээний төрийн томоохон ууган их сургуулиудын нэг болон өргөжжээ. ХААИС нь байгууллагдсан цагаасаа эхлэн нийт 30.000 орчим дээд боловсролтой мэргэжилтэн бэлтгэн гаргажээ.
        </p>
        <div class="item item-5">
            <img src="img/haais1.png" alt="elselt"  width="270px" height="230" >
        </div>
        <div class="item item-3"><img class="img" src="img/haais2.jpg" alt="elselt" width="270px" height="230" ></div>
        <div class="item item-4"><img src="img/haais3.jpg" alt="elselt" width="270px" height="230" ></div>
        </div>
        <!-- Босоо мэдээлэл -->
        <div class="item item-2"><h3 style="text-align: center;">ХААИС тоон мэдээлэл</h3>
            <br><br>
            <div 
            style="font-size:2.5em; color:#8bc34a; padding-right:0rem; font-family: Arial;">
            <b> 1 </b></div>
            <p class="list3"> Байгуулагдсан он:  1958
               </p> <br> <br> <br>
              <hr>
              <br>
              <div style="font-size:2.5em; color:#8bc34a; padding-right:0rem; font-family: Arial;">
            <b> 2 </b></div>
            <p class="list3">Суралцагчид</p> 
               <ul class="list2">
                  <li>Бакалавр оюутан: 5468</li>
                  <li>Магистрант оюутан: 2844</li> 
                  <li>Доктрант оюутан: 673</li> 
                  <li>Хэлний бэлтгэл:</li> 
                  <li>Гадаад оюутан:</li> 
               </ul> 
               <br> 
               <hr>
               <br>
        <div style="font-size:2.5em; color:#8bc34a; padding-right:0rem; font-family: Arial;">
               <b> 3 </b></div>
               <p class="list3">Нийт багш, ажиллагсад</p> 
               <ul class="list2">
                  <li>Үндсэн багш: 405</li>
                  <li>Цагийн багш: </li> 
                  <li>Бусад багш ажилтнууд: </li> 
                 
               </ul>
        </div> 
    </section> 
</section>  

    
 <!-- Холбоо барих хэсэг /Хөл мэдээлэл/ -->
<section id="container4">
    <div class="item2 item-5"><img src="img/logo-mb-2.png" > </div>
    <div class="item2 item-6"><h3 style="color: white;">Бүрэлдэхүүний сургуулиуд</h3> 
      
     <p><a class="holboo" href="https://muls.edu.mn">Мал эмнэлгийн сургууль</a></p>
     <p><a class="holboo" href="https://muls.edu.mn">Мал аж ахуй, биотехнологийн сургууль</a></p>
     <p><a class="holboo" href="https://muls.edu.mn">Инженер технологийн сургууль</a></p>
     <p><a class="holboo" href="https://muls.edu.mn">Агроэкологийн сургууль</a></p>
     <p><a class="holboo" href="https://muls.edu.mn">Эдийн засаг бизнесийн сургууль</a></p>
     <p><a class="holboo" href="https://muls.edu.mn">Магистр, докторын сургууль</a></p>
     <p><a class="holboo" href="https://muls.edu.mn">Агроэкологи бизнесийн сургууль</a></p>
     <p><a class="holboo" href="https://muls.edu.mn">Хөдөө аж ахуйн эдийн засаг, инноваци хөгжлийн төв</a></p>
    </div> 
    <div class="item2 item-7"><h3 style="color: white;">Холбоо барих</h3> 
      
        <p class="holboo" >&#9829; 17024 Улаанбаатар, Хан-Уул дүүрэг, Зайсан</p>
        <p class="holboo" >&phone; 976-75107777, 976-11341377</p>
        <p class="holboo" >&#x2709; info@muls.edu.mn</p>
        <p class="holboo" >&#x2709; admin@muls.edu.mn</p>
        <p><a class="holboo" href="https://www.facebook.com">&#9755; Facebook хаяг</a></p>
        <p><a class="holboo" href="https://www.youtube.com">&#9755; Youtube хаяг</a></p>
        <p><a class="holboo" href="https://www.twitter.com/">&#9755; Twitter хаяг</a></p>
        
       </div> 
</section> 
<!-- Footer хэсэг -->
<footer id="footer">
    <p class="text-center">Copyright &copy; Зохиогчийн эрхээр хамгаалагдсан 2020 он</p>
  </footer>  
</body>
</html